...
Scenario Outline: FAB-5797: Test chaincode basic ops with cc name in mixedcases chars, for <type> orderer
    ...
    And an admin deploys chaincode at path "../../fabric-test/chaincodes/example02/go/cmd" with args ["init","a","1000","b","2000"] with name "<ccName>"
...
Scenario: FAB-4717: FAB-5663, chaincode ex05 to ex02 on different channels
  ...
  When an admin sets up a channel
  And an admin deploys chaincode at path "../../fabric-test/chaincodes/example05/go/cmd" with args ["init","sum","0"] with name "myex05"
  When an admin sets up a channel named "channel2"
  And an admin deploys chaincode at path "../../fabric-test/chaincodes/example02/go/cmd" with args ["init","a","1000","b","2000"] with name "myex02_b" on channel "channel2"
...
Scenario: FAB-4720: FAB-5663, chaincode ex05 to ex02, on same channel, pass bad channnel name
  ...
  When an admin sets up a channel
  And an admin deploys chaincode at path "../../fabric-test/chaincodes/example05/go/cmd" with args ["init","sum","0"] with name "myex05"
  When an admin deploys chaincode at path "../../fabric-test/chaincodes/example02/go/cmd" with args ["init","a","1000","b","2000"] with name "myex02_b"
...
Examples:
 ...
 #  | github.com/hyperledger/fabric-test/chaincodes/example02/go/cmd | GOLANG   | with tls    |
    |          ../../fabric-test/chaincodes/example02/go/cmd         | GOLANG   | without tls |
