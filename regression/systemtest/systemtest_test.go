	...
	*/
	Describe("Kafka_Couchdb_TLS", func() {
		It("Kafka_Couchdb_TLS", func() {
			...
			inputSpecPath = "../../tools/operator/testdata/kafka-couchdb-tls/samplecc-8MB-input.yml"
			By("Step 14: Sending 8 MB invoke using samplecc")
			err = testclient.Testclient("invoke", inputSpecPath)
			Expect(err).NotTo(HaveOccurred())

			inputSpecPath = "../../tools/operator/templates/input.yaml"

			By("Step 15: Migrating to etcdraft")
			...
			By("Step 16: Sending invokes for samplecc")
			err = testclient.Testclient("invoke", inputSpecPath)
			Expect(err).NotTo(HaveOccurred())

			By("Step 17: Sending queries for samplecc")
			err = testclient.Testclient("query", inputSpecPath)
			Expect(err).NotTo(HaveOccurred())
