...
export class Workspace {
  ...
  public async getConnection(org: Org, identityName: string): Promise<Gateway> {
    ...
    if (
      !this.connections.has(org.name) ||
      !this.connections.get(org.name)!.has(identityName)
    ) {
      logger.debug(
        `Creating new gateway for organisation ${org.name} identity ${identityName}`
      );
    ...
    logger.debug(
      `Using existing gateway for organisation ${org.name} identity ${identityName}`
    );

    return this.connections.get(org.name)!.get(identityName)!;
  }

  public namespaceOrg(org: string) {
    let rid = this._runuid ? this._runuid : "";
    return org.replace("{UID}", rid);
  }

  public setInfrastuctureProvider(name: string, ctx?: any) {
